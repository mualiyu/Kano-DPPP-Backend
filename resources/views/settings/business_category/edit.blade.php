...
@section('content')
     <!-- Page content-->
          <div class="col-lg-9 pt-4 pb-2 pb-sm-4">
            <h1 class="h2 mb-4">Edit Bussiness Category</h1>
            <!-- Basic info-->
            <section class="card border-0 py-1 p-md-2 p-xl-3 p-xxl-4 mb-4">
              <form action="{{route('update_b_c', ['id'=>$business_category->id])}}" method="POST">
                @csrf
                <div class="card-body">
                  ...
                  <div class="row g-3 g-sm-4 mt-0 mt-lg-2">
                    <div class="col-sm-12 col-12">
                      ...
                      <input class="form-control" type="text" name="name" value="{{$business_category->name}}" id="fjn">
                    </div>
                    <hr>
                    <div class="col-12">
                    ...
                    <div class="row subbb">
                      <?php $i=1; ?>
                      @if (count($business_category->bussiness_sub_categories))
                        @foreach ($business_category->bussiness_sub_categories as $bsc)
                        <div class="col-sm-6">
                          <label class="form-label" for="sub">Sub Category({{$i}})</label>
                          <input type="hidden" name="sub_id[{{$i}}]" value="{{$bsc->id}}">
                          <input required class="form-control" type="text" value="{{$bsc->name}}" id="sub" name="sub_name[{{$bsc->id}}]">
                        </div>
                        <?php $i++; ?>
                        @endforeach
                      @endif
...
@section('script')
  <script>
    ...
    head.setAttribute("name", "sub_new_name["+b+"]");
