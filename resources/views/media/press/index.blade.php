...
@section('content')
    ...
    <div class="col-lg-9 pt-4 pb-2 pb-sm-4">
      <div class="d-flex align-items-center mb-4">
        <h1 class="h2 mb-0">Press Release</h1>
          <div class="ms-auto">
              <a class="btn btn-sm btn-primary ms-auto" style="float: right;" href="{{route('show_create_press')}}">Add Press Release</a>
          </div>
      </div>
            ...
            @if (count($press) > 0)
                @foreach ($press as $n)
                    <tr>
                        ...
                        <td>
                            <a  onclick="event.preventDefault(); window.location.href='/web-admin/perss-release/{{$n->id}}/press-release-info';" class="nav-link text-default fs-sm fw-normal text-primary py-1 pe-0 ps-1 ms-2" type="button" data-bs-toggle="tooltip" aria-label="View">
                                View
                            </a>
                            ...
                            <form id="b-form{{$n->id}}" action="{{route('delete_press', ['pressRelease'=>$n->id])}}" method="POST" class="d-none">
                                @csrf
                            </form>
                        </td>
                    </tr>
