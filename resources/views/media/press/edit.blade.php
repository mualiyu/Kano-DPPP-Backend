     ...
     <!-- Page content-->
          <div class="col-lg-9 pt-4 pb-2 pb-sm-4">
            ...
            <section class="card border-0 py-1 p-md-2 p-xl-3 p-xxl-4 mb-4">
              <form action="{{route('update_press', ['pressRelease'=>$pressRelease->id])}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                  <div class="d-flex align-items-center mt-sm-n1 pb- mb-0 mb-lg-1 mb-xl-3">
                      <p>News information below</p>

                  </div>
                  <div class="row g-3 g-sm-4 mt-0 mt-lg-2">
                    <div class="col-sm-12">
                      <label class="form-label" for="jn">Title*</label>
                      <input class="form-control pe-5" type="text" name="title" value="{{$pressRelease->title}}" id="fjn" required>
                    </div>

                    <div class="col-sm-6">
                      <label class="form-label" for="description">Description</label>
                      <textarea class="form-control" rows="4" placeholder="Write Description" id="description" name="desc">{{$pressRelease->description ?? ""}}</textarea>
                    </div>
                    {{-- <div class="col-sm-2">
                    ...
                    <div class="col-sm-6">
                        <div style="display:flex;">
                            ...
                            style="width: 80px; height: 80px;" src="/storage/press/{{$pressRelease->image}}" 
                            alt="No Image"> <p style="font-size: 12px;">{{$pressRelease->image}}</p>
                        </div>
                    ...
                    <div class="row contt">
                        @foreach ($pressRelease->press_contents as $nc)   
                        <div class="col-sm-1">
                          <br>
                          <br>
                          <br>
                        ...
                        <div class="col-sm-5">
                          ...
                          <div style="display:flex;">
                              <img class="d-flex flex-column justify-content-end position-relative overflow-hidden rounded-circle bg-size-cover bg-position-center" 
                              style="width: 80px; height: 80px;" src="/storage/press/{{$nc->image}}" 
                        ...
                        <div class="col-sm-6">
                          ...
                          <textarea class="form-control" rows="7" placeholder="Add content" id="content" name="n_content[{{$nc->id}}]">{{$nc->content}}</textarea>
                        </div>
                        <br>
                        <hr>
                        <br>
              ...
              @foreach ($pressRelease->press_contents as $nc)   
                <form id="n-form-{{$nc->id}}" action="{{route('delete_press_cont', ['cont'=>$nc->id])}}" method="POST">
                  @csrf
                </form>  
              @endforeach
            </section>
